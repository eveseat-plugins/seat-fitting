...
function drawLevelBox(neededLevel, currentLevel, skillName, rank) {
    ...
    graph += '<td style="width: 11em"><div style="background-color: transparent; text-align: center; height: 1.35em; letter-spacing: 2.25px;" class="pull-right">';

    if (currentLevel >= neededLevel) {
        for (let i = 0; i < neededLevel; i++) {
            graph += '<span class="fa fa-square " style="vertical-align: text-top; color: #5ac597;"></span>';
        }
        for (let i = neededLevel; i < currentLevel; i++) {
            graph += '<span class="fa fa-square text-green" style="vertical-align: text-top"></span>';
        }
        for (let i = 0; i < (5 - currentLevel); i++) {
            graph += '<span class="fa fa-circle text-green" style="vertical-align: text-top"></span>';
        }
    } else {
        ...
        for (let i = 0; i < (neededLevel - currentLevel); i++) {
            graph += '<span class="fa fa-circle text-danger" style="vertical-align: text-top"></span>';
        }
    ...
    graph += '</div><span class="pull-right"><small>' + trainingtime + '</small> </span>';
