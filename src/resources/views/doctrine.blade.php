    ...
    <div class="card card-primary card-solid">
                ...
                <select id="doctrineSpinner" class="form-control mr-3">
                    ...
                    @foreach ($doctrine_list as $doctrine)
                        <option value="{{ $doctrine['id'] }}"
                                @if($doctrine_id == $doctrine['id']) selected @endif>{{ $doctrine['name'] }}</option>
                    @endforeach
                </select>
                <div class="input-group-btn">
                    @can('fitting.create')
                        @if (!empty($doctrine_list))
                            <button type="button" id="editDoctrine" class="btn btn-warning btn-sm" disabled="disabled"
                                    data-id="" data-toggle="modal" data-target="#addDoctrine" data-toggle="tooltip"
                                    data-placement="top" title="Edit Doctrine" inactive>
                        <span class="fas fa-edit text-white"></span>
                    </button>
                            <button type="button" id="deleteDoctrine" class="btn btn-danger btn-sm" disabled="disabled"
                                    data-id="" data-toggle="tooltip" data-placement="top" title="Delete Doctrine">
        ...
        <div class="card-footer text-muted">
            Plugin maintained by <a
                    href="{{ route('cryptafitting::about') }}"> {!! img('characters', 'portrait', 96057938, 64, ['class' => 'img-circle eve-icon small-icon']) !!}
