...
@section('left')
    <div class="card card-primary card-solid">
            ...
            @can('fitting.create')
                <div class="card-tools pull-right">
                    <button type="button" class="btn btn-xs btn-tool" id="addFitting" data-toggle="tooltip"
                            data-placement="top" title="Add a new fitting">
                ...
                @if (count($fitlist) > 0)
                    @foreach($fitlist as $fit)
                        <tr class="fitid" data-id="{{ $fit['id'] }}">
                            <td><img src='https://images.evetech.net/types/{{ $fit['typeID'] }}/icon?size=32' height='24'/>
                            </td>
                            <td>{{ $fit['shiptype'] }}</td>
                            <td>{{ $fit['fitname'] }}</td>
        ...
        <div class="card-footer text-muted">
            Plugin maintained by <a
                    href="{{ route('fitting.about') }}"> {!! img('characters', 'portrait', 96057938, 64, ['class' => 'img-circle eve-icon small-icon']) !!}
                Crypta Electrica</a>. <span class="float-right snoopy" style="color: #fa3333;"><i
    ...
    <script type="application/javascript">
        ...
        $('#fitlist').on('click', '#deletefit', function () {
            ...
            $.ajax({
                ...
                if (result) {
                    $buy = result.buy.toLocaleString();
                    $sell = result.sell.toLocaleString();
                    $volume = result.volume.toLocaleString();

                    $('#current_appraisal').html($buy + " / " + $sell + " (ISK)" + " - " + $volume + "m3");
                }
            });
        });
                ...
                for (slot in result) {
                    ...
                    if (slot.indexOf('HiSlot') >= 0)
                        $('#highSlots').find('tbody').append(
                            "<tr><td><img src='https://images.evetech.net/types/" + result[slot].id + "/icon?size=32' height='24' /> " + result[slot].name + "</td></tr>");
                    ...
                    if (slot.indexOf('dronebay') >= 0) {
                        for (item in result[slot])
                            $('#drones').find('tbody').append(
                                "<tr><td><img src='https://images.evetech.net/types/" + item + "/icon?size=32' height='24' /> " + result[slot][item].name + "</td><td>" + result[slot][item].qty + "</td></tr>");
                    }
                }
            }
        }
